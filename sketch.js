...
class Ball {
    ...
    draw() {
        this.ctx.fillStyle = 'rgb(247,237,226)'
        if (this.done) {
    ...
    update() {
        if (380 < this.x && 420 > this.x && 445 < this.y && 485 > this.y) {
            this.done = true
            this.index++
    ...
    calcFitness() {
        var d = Math.sqrt((this.x - 400) ** 2 + (this.y - 465) ** 2)
        this.fitness = Math.max(0, 1 - d / 500)
    }
}
...
function animateLoop() {
    ...
    ctx.fillStyle = 'rgb(247,237,226)'
    ctx.fillRect(380, 485, 40, 40)
    ctx.fillStyle = 'rgb(0,0,0)'
    ctx.font = '30px Arial'
    ctx.fillText('Generation: ' + generation.toString(), 30, 30)
    ctx.fillText('Avg fitness: ' + avg_fitness.toFixed(2).toString(), 30, 70)
...
// set listeners for change in value of slider
document.getElementById('vel').onchange = (val) => (VEL = val.target.value * 1)

document.getElementById('mutation_rate').onchange = (val) =>
    (MUTATION_RATE = val.target.value * 1)